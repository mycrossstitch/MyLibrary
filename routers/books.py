...
@router.post("", response_model=SBook, status_code=status.HTTP_201_CREATED, summary="Добавить одну книгу")
...
@router.get("/is_read", status_code=status.HTTP_200_OK, summary="Получить книги по статусу прочтения")
async def get_books_by_read_status(session: SessionDep,  is_read: bool = False): # По умолчанию False (непрочитанные)
    books = await BookRepository.find_by_is_read(is_read, session)
    return books

@router.get("/search",
    response_model=List[SBook],
    status_code=status.HTTP_200_OK, summary="Поиск книг по параметрам")
async def search_books(session: SessionDep,
...
@router.put("/{id}", response_model=SBook, status_code=status.HTTP_200_OK, summary="Обновить книгу по ID")
